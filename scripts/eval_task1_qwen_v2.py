"""
Evaluación de Qwen3 (v2) vía HuggingFace Transformers para clasificación retórica
de 8 clases (Tarea 1).
...
Nota: CoT NO se aplica a Qwen (max_new_tokens quedaría en 120+ → offload CPU+disco en RTX 4070).
Se aplica solo a Gemini v2 (sin restricción de VRAM).

Modos:
  - zero_shot  : definiciones contrastivas + CoT, sin ejemplos
  - few_shot   : definiciones contrastivas + CoT + ejemplos targeted (80 palabras)
  - both       : ejecuta ambos (default)

Uso:
  python scripts/eval_task1_qwen_v2.py
  python scripts/eval_task1_qwen_v2.py --model Qwen/Qwen3-8B --mode zero_shot
  python scripts/eval_task1_qwen_v2.py --mode few_shot

Requisitos:
  - pip install transformers accelerate
  - GPU recomendada (RTX 4070 12 GB soporta Qwen3-8B en bfloat16)
"""
...
# Sin CoT para Qwen (limitación VRAM): respuesta directa igual que v1
...
def build_fewshot_block(train_df: pd.DataFrame, rng: random.Random,
                        n_per_class: int = 1, max_words: int = 80) -> str:
    """
    Selecciona ejemplos con mayor puntuación de keywords discriminativas por clase.
    max_words=80 para no desbordar VRAM en RTX 4070.
    """
...
class QwenClassifier:
    ...
    def classify(self, text: str, system_prompt: str,
        ...
        """Clasifica un fragmento (sin CoT). Devuelve (etiqueta_parseada, latencia_s)."""
        ...
        with torch.no_grad():
            output_ids = self.model.generate(
                **inputs,
                max_new_tokens=15,        # solo el código de etiqueta
...
def evaluate(classifier: QwenClassifier, test_df: pd.DataFrame,
    ...
    return {
        ...
        "version": "v2",
        "improvements": ["contrastive_defs", "targeted_fewshot", "truncate_300_100"],
...
def main():
    ...
    # Few-shot: targeted + 80 palabras (límite VRAM RTX 4070)
    few_shot_block = build_fewshot_block(train_df, rng, n_per_class=1, max_words=80)
    ...
    for mode in modes:
        ...
        out_path = RESULTS_DIR / f"results_task1_{model_slug}_v2_{mode}.json"
