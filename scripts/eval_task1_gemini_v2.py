"""
Evaluación de Gemini Flash (v2) para clasificación retórica de 8 clases (Tarea 1).

Mejoras respecto a v1:
  1. Definiciones contrastivas para pares confusos (BACK/INTRO, RES/DISC, INTRO/CONTR)
  2. Ejemplos few-shot seleccionados con keywords discriminativas por clase (targeted)
  3. Truncación de chunks a 300+100 palabras (head+tail) antes del prompt
  4. Chain-of-thought: el modelo razona 1-2 oraciones y da ETIQUETA: <código> al final

Modos:
  - zero_shot  : definiciones contrastivas + CoT, sin ejemplos
  - few_shot   : definiciones contrastivas + CoT + ejemplos targeted
  - both       : ejecuta ambos (default)

Uso:
  python scripts/eval_task1_gemini_v2.py
  python scripts/eval_task1_gemini_v2.py --mode zero_shot
  python scripts/eval_task1_gemini_v2.py --mode few_shot
...
REQUEST_DELAY_S = 4.2  # ~14 req/min para dejar margen en el tier gratuito
...
Pares frecuentemente confundidos — distinciones clave:
• BACK vs INTRO: BACK cita y describe trabajos de OTROS autores ("estudios previos han demostrado...", "según García et al.", "la literatura indica..."). INTRO habla del PROPIO estudio: plantea el problema, la brecha de conocimiento o los objetivos del trabajo actual ("este trabajo busca...", "se propone...", "el objetivo es...").
• RES vs DISC: RES reporta hallazgos crudos con datos o métricas sin interpretar ("se obtuvo un F1 de 0.87", "el grupo A mostró mayor frecuencia"). DISC analiza qué significan esos datos, los compara con estudios previos o extrae conclusiones ("estos resultados sugieren que...", "en contraste con lo reportado por...", "esto indica que...").
• INTRO vs CONTR: INTRO contextualiza el problema de forma general ("existe una brecha en el conocimiento sobre..."). CONTR enuncia explícitamente la propuesta original ("la principal contribución de este trabajo es...", "proponemos un nuevo método...", "se presenta por primera vez...").
...
# CoT: el modelo razona brevemente y luego da la etiqueta en formato estructurado
USER_TEMPLATE = ('Fragmento: "{text}"\n\n'
                 'Razona brevemente (1-2 oraciones) qué función retórica cumple este fragmento. '
                 'Luego, en la última línea, escribe únicamente:\n'
                 'ETIQUETA: <código>')
...
def truncate_text(text: str, head_words: int = 300, tail_words: int = 100) -> str:
    """Trunca el texto a head+tail palabras (head+tail tokenization en palabras)."""
...
def parse_label(response: str) -> str | None:
    """Extrae la etiqueta canónica de la respuesta del modelo (parseo directo)."""
...
def parse_label_cot(response: str) -> str | None:
    """Para respuestas CoT: busca 'ETIQUETA: CÓDIGO' y extrae el código."""
    match = re.search(r'etiqueta\s*:\s*([A-Za-z]+)', response, re.IGNORECASE)
    if match:
        candidate = match.group(1).strip().upper()
        if candidate in set(LABEL_ORDER):
            return candidate
        if candidate.lower() in ALIAS_MAP:
            return ALIAS_MAP[candidate.lower()]
    # Fallback al parseo normal si no encuentra el patrón
    return parse_label(response)


def split_by_document(df: pd.DataFrame, train_ratio: float, val_ratio: float,
                      seed: int = 42):
    """Divide por documento, sin solapamiento entre splits."""
...
# ── Few-shot targeted ──────────────────────────────────────────────────────────
# Keywords discriminativas por clase para seleccionar el ejemplo más representativo
DISCRIMINATIVE_KEYWORDS = {
    "BACK": ["estudios previos", "investigaciones anteriores", "según", "et al",
             "han demostrado", "la literatura", "trabajos previos", "revisión"],
    "INTRO": ["objetivo", "esta investigación", "este trabajo", "busca", "se propone",
              "se plantea", "problemática", "brecha", "necesidad", "el presente estudio"],
    "RES":  ["%", "accuracy", "precisión", "se obtuvo", "mostró", "tabla", "figura",
             "p <", "p<", "media", "desviación", "coeficiente", "score", "f1"],
    "DISC": ["sugiere", "implica", "en comparación", "contrario a", "estos resultados",
             "consistente con", "indica que", "explicar", "posible que", "interpretación"],
    "CONTR": ["contribución", "propone", "nuevo método", "principal aporte",
              "por primera vez", "se presenta", "aportamos", "novedad", "innovación"],
    "LIM":  ["limitación", "restricción", "sesgo", "no es posible", "supuesto",
             "generalizar", "tamaño de muestra", "alcance", "no permite"],
    "CONC": ["concluimos", "trabajo futuro", "en conclusión", "los hallazgos",
             "perspectivas", "líneas futuras", "en síntesis", "se concluye"],
    "METH": ["metodología", "muestra", "instrumento", "procedimiento", "variables",
             "análisis estadístico", "recolección", "participantes", "diseño"],
}


def build_fewshot_examples(train_df: pd.DataFrame, rng: random.Random,
                           n_per_class: int = 1, max_words: int = 400) -> str:
    """
    Selecciona ejemplos con mayor puntuación de keywords discriminativas por clase.
    Si hay empate, elige aleatoriamente entre los mejores.
    """
    lines = ["\nEjemplos de clasificación:\n"]
    for label in LABEL_ORDER:
        subset = train_df[train_df["etiqueta_anotador"] == label].copy()
        keywords = DISCRIMINATIVE_KEYWORDS.get(label, [])
        if keywords:
            subset["_score"] = subset["texto"].apply(
                lambda t: sum(1 for kw in keywords if kw in str(t).lower())
            )
            top_score = subset["_score"].max()
            candidates = subset[subset["_score"] == top_score]
        else:
            candidates = subset
        ...
        for _, row in samples.iterrows():
            snippet = " ".join(str(row["texto"]).split()[:max_words])
...
class GeminiClassifier:
    ...
    def classify(self, text: str, system_prompt: str,
        ...
        """Clasifica un fragmento con CoT. Devuelve (etiqueta_parseada, latencia_s)."""
        from google.genai import types

        truncated = truncate_text(text)
        user_content = USER_TEMPLATE.replace("{text}", truncated)
            ...
            try:
                ...
                response = self._client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        temperature=0.0,
                        max_output_tokens=200,   # CoT necesita más tokens que solo la etiqueta
                        thinking_config=types.ThinkingConfig(thinking_budget=0),
                    ),
                )
                latency = time.perf_counter() - t0
                raw = response.text.strip() if response.text else ""
                return parse_label_cot(raw), latency
...
def evaluate(classifier: GeminiClassifier, test_df: pd.DataFrame,
    ...
    return {
        ...
        "model": GEMINI_MODEL,
        "version": "v2",
        "improvements": ["contrastive_defs", "targeted_fewshot", "truncate_300_100", "cot"],
...
def main():
    ...
    # Few-shot: ejemplos targeted (max 400 palabras por ejemplo, sin restricción de VRAM)
    few_shot_block = build_fewshot_examples(train_df, rng, n_per_class=1, max_words=400)
    ...
    for mode in modes:
        ...
        out_path = RESULTS_DIR / f"results_task1_gemini_v2_{mode}.json"
